...
void RTSP::startRead(void)
{
    m_timer = new QTimer;
    connect(this->m_timer, &QTimer::timeout, this, &RTSP::timeoutslot);
    m_timer->start(20);
}

void RTSP::timeoutslot(void)
{
    if (this->myffmpeg_readframe() < 0)
    {
        qDebug("read frame error");
    }
}

int RTSP::myffmpeg_init(void)
{
    int i;
    int ret = -1;
...
void RTSP::myffmpeg_destory(void)
{
    // 释放输出缓冲区
        av_free(m_OutBuffer);
...
int RTSP::myffmpeg_readframe(void)
{
    int ret = -1;
    int getPicture = 0;
...
void RTSP::myffmpeg_setRTSPurl(QString url)
{
    this->m_rtspUrl = url;
}
