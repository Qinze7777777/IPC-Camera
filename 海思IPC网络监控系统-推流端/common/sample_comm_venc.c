...
int flag_push = 0;
int flag_getmediainfo = 0;
EASY_CHANNEL_HANDLE g_channelHandle;
EASY_RTSPSERVER_MEDIA_INFO_T g_mediaInfo;

static MP4TrackId video = 0;
static MP4FileHandle mp4FileHd = NULL;
											...
											void *userPtr, void *channelPtr)
{
    ...
    if (EASY_CHANNEL_OPEN_STREAM == serverStatus)
	{
        ...
        printf("set\n");
        // mediaInfo->vpsLength = g_mediaInfo.vpsLength;
        // mediaInfo->spsLength = g_mediaInfo.spsLength;
        // mediaInfo->ppsLength = g_mediaInfo.ppsLength;
        // memcpy(mediaInfo->vps, g_mediaInfo.vps, mediaInfo->vpsLength);
        // memcpy(mediaInfo->sps, g_mediaInfo.sps, mediaInfo->spsLength);
        // memcpy(mediaInfo->pps, g_mediaInfo.pps, mediaInfo->ppsLength);
        // g_mediaInfo.videoCodec = EASY_SDK_VIDEO_CODEC_H265;
        g_mediaInfo.videoCodec = EASY_SDK_VIDEO_CODEC_H264;
        g_mediaInfo.videoFps = 30;
        g_mediaInfo.videoQueueSize = 4096 * 1024;
        *mediaInfo = g_mediaInfo;
        EasyRtspServer_CreateChannel(resourceName, &g_channelHandle, NULL);
        *channelHandle = g_channelHandle;
    }
    if (EASY_CHANNEL_START_STREAM == serverStatus)
	{
        printf("start push\n");
		flag_push = 0x01;		//开始发送流
	}
	if (EASY_CHANNEL_STOP_STREAM == serverStatus)
	{
        printf("end push\n");
		flag_push = 0x00;		//停止发送流
	}
...
static HI_VOID SAMPLE_COMM_VENC_SendH265Frame(VENC_STREAM_S *stStream)
{
    int j;
    for (j = 0; j < stStream->u32PackCount; j++)
    {
        if (stStream->pstPack[j].DataType.enH265EType == H265E_NALU_VPS)
        {
            g_mediaInfo.vpsLength = stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset;
            memcpy(g_mediaInfo.vps, stStream->pstPack[j].pu8Addr + stStream->pstPack[j].u32Offset, g_mediaInfo.vpsLength);
        }
        else if (stStream->pstPack[j].DataType.enH265EType == H265E_NALU_SPS)
        {
            g_mediaInfo.spsLength = stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset;
            memcpy(g_mediaInfo.sps, stStream->pstPack[j].pu8Addr + stStream->pstPack[j].u32Offset, g_mediaInfo.spsLength);
        }
        else if (stStream->pstPack[j].DataType.enH265EType == H265E_NALU_PPS)
        {
            g_mediaInfo.ppsLength = stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset;
            memcpy(g_mediaInfo.pps, stStream->pstPack[j].pu8Addr + stStream->pstPack[j].u32Offset, g_mediaInfo.ppsLength);
        }
        else if (stStream->pstPack[j].DataType.enH265EType == H265E_NALU_SEI)
        {
            g_mediaInfo.seiLength = stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset;
            memcpy(g_mediaInfo.sei, stStream->pstPack[j].pu8Addr + stStream->pstPack[j].u32Offset, g_mediaInfo.seiLength);
        }
        else
        {
            EASY_AV_Frame f;
            if (flag_push && stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset > 0)
            {
                f.pBuffer = stStream->pstPack[j].pu8Addr + stStream->pstPack[j].u32Offset;
                f.u32AVFrameFlag = EASY_SDK_VIDEO_FRAME_FLAG; 
                f.u32AVFrameLen = stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset;
                if ( stStream->pstPack[j].DataType.enH265EType == H265E_NALU_ISLICE || stStream->pstPack[j].DataType.enH265EType == H265E_NALU_IDRSLICE)
                {
                    printf("I\n");
                    f.u32VFrameType = EASY_SDK_VIDEO_FRAME_I;
                }
                if ( stStream->pstPack[j].DataType.enH265EType == H265E_NALU_BSLICE)
                {
                    printf("B\n");
                    f.u32VFrameType = EASY_SDK_VIDEO_FRAME_B;
                }
                if ( stStream->pstPack[j].DataType.enH265EType == H265E_NALU_PSLICE)
                {
                    printf("P\n");
                    f.u32VFrameType = EASY_SDK_VIDEO_FRAME_P;
                }
                f.u32TimestampSec = stStream->pstPack[j].u64PTS / 1000000;       //取秒部分
                f.u32TimestampUsec = stStream->pstPack[j].u64PTS % 1000000;      //取微秒部分
                EasyRtspServer_PushFrame(g_channelHandle, &f); 
            }
        }
    }
}
static HI_VOID SAMPLE_COMM_VENC_SendH264Frame(VENC_STREAM_S *stStream)
{
    int j;
    for (j = 0; j < stStream->u32PackCount; j++)
    {
        ...
        else if (stStream->pstPack[j].DataType.enH264EType == H264E_NALU_PPS)
        {
            g_mediaInfo.ppsLength = stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset;
            memcpy(g_mediaInfo.pps, stStream->pstPack[j].pu8Addr + stStream->pstPack[j].u32Offset, g_mediaInfo.ppsLength);
        }
        else if (stStream->pstPack[j].DataType.enH264EType == H264E_NALU_SEI)
        {
            g_mediaInfo.seiLength = stStream->pstPack[j].u32Len - stStream->pstPack[j].u32Offset;
            memcpy(g_mediaInfo.sei, stStream->pstPack[j].pu8Addr + stStream->pstPack[j].u32Offset, g_mediaInfo.seiLength);
        }
...
HI_VOID SAMPLE_COMM_VENC_CloseMP4(void)
{
    if (mp4FileHd != NULL && video != 0)
    {
        MP4Close(mp4FileHd, 0);
        mp4FileHd = NULL;
        video = 0;
        printf("close mp4\n");
    }
...
static HI_VOID SAMPLE_COMM_VENC_SaveH264ToMP4(VENC_STREAM_S *stStream)
{
    uint8_t *pData;
    int length;
    int syncFlag = 0;
    ...
    if (mp4FileHd == NULL)
    {
        ...
        sprintf(filename, "/mnt/sdcard/%d-%d-%d-%d-%d-%d.mp4", tm_info->tm_year + 1900, tm_info->tm_mon + 1, tm_info->tm_mday, tm_info->tm_hour, tm_info->tm_min, tm_info->tm_sec);
        mp4FileHd = MP4Create(filename, 0);
        if (mp4FileHd == MP4_INVALID_FILE_HANDLE)	
        {
			printf("open file fialed.\n");
			return;
		}
        MP4SetTimeScale(mp4FileHd, 90000);
    }
    
    if (stStream != NULL && stStream->u32Seq > 30)
    {
        for (size_t i = 0; i < stStream->u32PackCount; i++)
        {
            length = stStream->pstPack[i].u32Len - stStream->pstPack[i].u32Offset;
            // pData = (stStream->pstPack[i].pu8Addr + stStream->pstPack[i].u32Offset);
            pData = (uint8_t *)malloc(length);
            memcpy(pData, stStream->pstPack[i].pu8Addr + stStream->pstPack[i].u32Offset, length);
            // printf("length:%d", length);
            if (stStream->pstPack[i].DataType.enH264EType == H264E_NALU_SPS && !spsFlag)
            {
                spsFlag = 1;
                printf("Write sps =================\n");
                video = MP4AddH264VideoTrack(mp4FileHd, 90000, 90000 / 30, 1280, 720,
                                            pData[4+1], //sps[1] AVCProfileIndication       
                                            pData[4+2], //sps[2] profile_compat
                                            pData[4+3], //sps[3] AVCLevelIndication
                                            3); // 4 bytes length before each NAL unit          起始码长度为4,这里要求填的是起始码字节长度减1,所以是3
                MP4SetVideoProfileLevel(mp4FileHd, 0x7F);
                MP4AddH264SequenceParameterSet(mp4FileHd, video, pData+4, length-4);
            }
            else if (stStream->pstPack[i].DataType.enH264EType == H264E_NALU_PPS && !ppsFlag) 
            {
                ppsFlag = 1;
                printf("Write pps =================\n");
                MP4AddH264PictureParameterSet(mp4FileHd, video, pData+4, length-4);
            }
            else if ((stStream->pstPack[i].DataType.enH264EType == H264E_NALU_IDRSLICE || stStream->pstPack[i].DataType.enH264EType == H264E_NALU_ISLICE || 
            stStream->pstPack[i].DataType.enH264EType == H264E_NALU_BSLICE  || stStream->pstPack[i].DataType.enH264EType == H264E_NALU_PSLICE) && video != 0)     //如果为I/P/B帧
            {
                // printf("Write sample =================\n");
                if (stStream->pstPack[i].DataType.enH264EType == H264E_NALU_IDRSLICE || stStream->pstPack[i].DataType.enH264EType == H264E_NALU_ISLICE)
                {
                    syncFlag = 1;
                }
                else
                ...
                pData[2] = (length - 4) >> 8;
                pData[3] = length - 4; 
                MP4WriteSample(mp4FileHd, video, pData, length, MP4_INVALID_DURATION, 0, syncFlag);
            }
            free(pData); 
        }

        if (stStream->u32Seq - offset > 3600)    //1分钟
        {
            offset += 3600;
            SAMPLE_COMM_VENC_CloseMP4();
            spsFlag = 0;
            ppsFlag = 0;
        }
    }
...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID* p)
{
    ...
    ******************************************/
    ...
    for (i = 0; i < s32ChnTotal; i++)
    {
        ...
        printf("payloadtype:%d", enPayLoadType[i]);
    ...
    //初始化rtsp
    EasyRtspServer_Startup(656, "video0", EASY_AUTHENTICATION_TYPE_NONE,  NULL, NULL, __EasyRtspServer_Callback, NULL);
    printf("rtsp://192.168.5.9:656/video0\n");
    /******************************************
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    // if (i == 0)
                    // {
                    //     SAMPLE_COMM_VENC_SendH265Frame(&stStream);
                    // }
                    //H264推流
                    if (i == 1)
                    {
                        SAMPLE_COMM_VENC_SendH264Frame(&stStream);
                    }
                    // // //保存为MP4文件
                    SAMPLE_COMM_VENC_SaveH264ToMP4(&stStream);   
#ifndef __HuaweiLite__
