...
int open_fb(void)
{
	int width,height,fd,index,fbValue;
	HIFB_COLORKEY_S stColorkey;

	width = 3840;
	height = 2160;
	fbValue = 8888;
	//info_msg("width = %d height = %d, fbValue = %d\n",width, height, fbValue);
	const char* fbNode = NULL;
	if(!fbNode)
		fbNode = "/dev/fb0";


	fd = open(fbNode, O_RDWR, 0);
	if (fd < 0) {
	...
	for (;;) {
		...
		if(ioctl(fd, FBIOPUT_COLORKEY_HIFB, &stColorkey) <0) {
			printf("FBIOPUT_COLORJEY_HIFB failed\n");
			break;
		}
