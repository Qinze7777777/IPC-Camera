...
int HH_OSD_SetOsdPosDefault()
{
	...
	{
		sprintf(pvideo_osd_status->Title,"%s",	 "HI3516DV300");
	}
...
int HH_OSD_Init()
{
	...
	for (nEncType = 0; nEncType < VIDEO_NUM; nEncType++) //  nEncType= 0
	{
		for (osdType = 0; osdType < HH_MAX_OSD_PER_GROUP; osdType++)  // 2 区域
		{
			...
			HH_OSD_SetOsdPosDefault();//获取默认值
			HH_OSD_GetOrg(nCh, nEncType, osdType, &osdOrg);//获取显示的坐标
			HH_OSD_GetColor(nCh, &osdColor);//获取颜色
			HH_OSD_GetTitle(nCh, nEncType, osdType, szTitle);//获取显示的文字内容
			...
			//2024年10月18日 nEncType手动的赋值成1 ，因为我不要vencch0那一路，我只要vencch1这一路
			//然后之所以这里改，是因为后面改会报错说不能修改静态变量
			nEncType = 1;
