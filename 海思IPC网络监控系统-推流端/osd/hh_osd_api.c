...
extern HH_VIDEO_RESOLUTION ipc_conf;//通过这里获取摄像头的信息，这里osd就弄成通用的

/*
其实就是实例化一个hh_osd的对象并填充
*/
...
int HI_OSD_Parse_OsdTitle(HH_OSD_LOGO *pOsdLogo)
{
	...
	while (pOsdLogo->szOsdTitle[index] != '\0' && pOsdLogo->nOsdLine <= HH_MAX_TITLE_LINE)
	{
		...
		else if (pOsdLogo->szOsdTitle[index] > 0x7F)//如果是中文字符，一个中文字符是占用两个char的
		{
...
/*
根据ipc_conf配置的视频源的大小来配置osd中的pOsdLogo的宽高和坐标
*/
int HI_OSD_Get_BmpSize(HH_OSD_LOGO *pOsdLogo)
{
	...
	pOsdLogo->nWidth = (pOsdLogo->nOsdFontW + pOsdLogo->nFontInt)* pOsdLogo->nOsdTitleMaxLen - pOsdLogo->nFontInt;//osd的区域宽度 = （字体的宽度+每个字体间的间距）*多少个字 -最后一个字的字间距 
	pOsdLogo->nHeight= pOsdLogo->nOsdFontH * pOsdLogo->nOsdLine;

	pOsdLogo->nWidth = (pOsdLogo->nWidth+1) & 0xFFE;//对齐操作
	pOsdLogo->nHeight= (pOsdLogo->nHeight+1)& 0xFFE;
	//COMM_SYSLOG(LOG_ERR,"test---------pOsdLogo->nWidth = %d \n",pOsdLogo->nWidth);

	if(vencWith == 720)
	{
		if(pOsdLogo->regHdl%2){	//index=1的title显示在右下角
			pOsdLogo->osdPos.nOrgX = vencWith - 200;
			pOsdLogo->osdPos.nOrgY = vencHeight - 40;

		}else{//index=0的time显示在左上角
			pOsdLogo->osdPos.nOrgX = 5;
			pOsdLogo->osdPos.nOrgY = 5;
	...
	else if(vencWith == 1920)
	{
		if(pOsdLogo->regHdl%2)
		{
			pOsdLogo->osdPos.nOrgX = vencWith - 20 ;
			pOsdLogo->osdPos.nOrgY = vencHeight - 10;
		}else
		{
			pOsdLogo->osdPos.nOrgX = 20;
			pOsdLogo->osdPos.nOrgY = 10;
		}
	}

	if (memcmp(&tmpLogo, pOsdLogo, sizeof(HH_OSD_LOGO)) == 0)//判断pOsdLogo是否修改成功
		return 0;
...
/*
申请了osd的bmp图片的内存buf，然后调用了海思的rgn的api完成overlay的初始化
*/
...
//#if defined(HI3516_ARCH) || defined(HI3518_ARCH)
	...
	pOsdLogo->pBmpBuf = malloc(nBmpSize);//存储生成的bmp图片的
	if (pOsdLogo->pBmpBuf == NULL)
	{
	...
	memset(pOsdLogo->pBmpBuf, 0x55, pOsdLogo->nWidth * pOsdLogo->nHeight * 2);//这个内存全部填充成0x55，这是一个魔数随便你定
	...
	stRgnAttr.unAttr.stOverlay.u32CanvasNum = 2; //hsx
	...
	if(HI_SUCCESS != s32Ret)
	{
		COMM_SYSLOG(LOG_ERR,"%s: HI_MPI_RGN_Create (%d) failed with %#x!\n", __FUNCTION__, pOsdLogo->regHdl, s32Ret);
...
/*
根据pOsdLogo中记录的osd要显示的内容，生成bmp要的数据（但是不是真正的要去生成一张bmp）
然后调用HI_MPI_RGN_SetBitMap将内容显示到osd中
*/
...
/*
检查从ipc_config读取的视频流的大小和pOsdLogo记录的是否一样的大小
如果是返回0 否则返回1
*/
...
int HI_OSD_Build(HH_OSD_LOGO *pOsdLogo)
{
	...
	HI_OSD_Parse_OsdTitle(pOsdLogo);//字符串解析
...
* 功能描述: 取得汉字的点阵
* 其他说明:用输入的qu和wei去查字模表，将查得到的字模输出到in字符串中	
*******************************************************************************/
